...
=====================================================
    ...
    Status: **SUPERSEDED by declarative image variants** — for File fields of a Content Block, replace this pattern with `headless.yaml <../Howto/DefineImageVariants.rst>`__. Kept as the historical reference for migrating existing blocks; production sites carry many copies of this file (13 on the site this extension was built for). One case is **not** yet replaceable: images inside Collection items — see the `migration guide <../Howto/MigrateLegacyThumbnails.rst>`__.
...
The declarative replacement produces the same `thumbnails` map next to the frozen `id/alt/title/publicUrl` shape: `Define image variants <../Howto/DefineImageVariants.rst>`__.
