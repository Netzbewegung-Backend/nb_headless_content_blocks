...
Symptom → cause → fix for problems in this extension's **own test setup** (composer version switches, `runTests.sh`, GitHub Actions/`act`). Problems when *using* the extension are covered in :ref:`Troubleshooting <troubleshooting>`.
...
**Symptom:** `Build/Scripts/runTests.sh -s functional` aborts with `TYPO3\TestingFramework\Core\Exception: Can not remove folder`.
